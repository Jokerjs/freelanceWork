...
jQuery(function(){
    ...
    var $queue = $('<div class="filelist"></div>').appendTo( $wrap.find('.queueList'));
    ...
    function addFile( file ){
        ...
        var fileName = file.name.split('.');
            ...
            if(fileName[0].length>15){
                file_Name = fileName[0].substr(0,10) + '...' + fileName[1];
            }else{
                file_Name=file.name
            }
        ...
        var $li =   //$('<li data-key="'+file.key+'"  data-src="'+file.src+'" data-sort="'+index+'" draggable="true" id="' + file.id + '" style="position:absolute;margin:0;cursor:move;width:'+thumbnailWidth+'px;height:'+thumbnailHeight+'px;left:'+imgLeft+'px;top:'+imgTop+'">' +
                $(
                    '<div id="' + file.id + '" class="item file-box">' +
                        '<div class="info">'+
                            '<div class="info_text">'+' <i class="fa fa-folder"></i>'+ file_Name + '</div>' +
    ...
    function updateStatus(){
        var text = '', stats;
        if( state == 'ready'){
            text = '选中'+fileCount + '个，共'+ WebUploader.formatSize( fileSize ) +'.';
...
function queueListShowli(arr){
    var $queueListShow = $(".queueList-show ul");
    var $Modal = $(
